...
def create_datastax_connection():
    ...
    with open("pdf_qa-token.json") as f:
        secrets = json.load(f)
...
def main():
    ...
    # Obtén la ruta del directorio temporal del sistema
    ...
    with st.sidebar:
        ...
        if st.button('Process'):
            ...
            with tempfile.NamedTemporaryFile(suffix='.pdf', dir=temp_dir, delete=False) as f:
                f.write(docs.getbuffer())
                with st.spinner('Processing'):
                    # Guarda el nombre del archivo temporal para usarlo más adelánte
